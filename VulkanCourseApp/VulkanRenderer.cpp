...
void VulkanRenderer::createMeshData()
{
	// Create a mesh
	// Vertex Data
...
void VulkanRenderer::createDescriptorSetLayout()
{
	...
	layoutCreateInfo.bindingCount = static_cast<uint32_t>(layoutBindings.size());  //Num binding infos
	layoutCreateInfo.pBindings = layoutBindings.data(); //Array of binding infos

	//Create descriptor set layout
	checkValidResult(vkCreateDescriptorSetLayout(mainDevice.logicalDevice, &layoutCreateInfo, nullptr, &descriptorSetLayout), "DESCRIPTOR SET LAYOUT CREATION FAILED");
}

void VulkanRenderer::createPushConstantRange()
{
	//Define push constant value
...
void VulkanRenderer::createGraphicsPipeline()
{
	...
	depthStencilStateCreateInfo.depthTestEnable = VK_TRUE; //Enable checking depth
	depthStencilStateCreateInfo.depthWriteEnable = VK_TRUE; //enable write to depth buffer
	depthStencilStateCreateInfo.depthCompareOp = VK_COMPARE_OP_LESS; //Comparison operation that allows overwrite
	depthStencilStateCreateInfo.depthBoundsTestEnable = VK_FALSE; //Depth bounds test
...
void VulkanRenderer::createCommandBuffers()
{
	...
	checkValidResult(vkAllocateCommandBuffers(mainDevice.logicalDevice, &cbAllocInfo, commandBuffers.data()), "COMMAND BUFFER ALLOCATION FAILED");
}
...
void VulkanRenderer::createDescriptorPool()
{
	...
	//Data to descriptor pool
	VkDescriptorPoolCreateInfo poolCreateInfo = {};
	poolCreateInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
	poolCreateInfo.maxSets = static_cast<uint32_t>(swapChainImages.size()); //Max num of Descriptor sets from pool
	poolCreateInfo.poolSizeCount = static_cast<uint32_t>(descriptorPoolSizes.size()); //Num of pool sizes being passed
	poolCreateInfo.pPoolSizes = descriptorPoolSizes.data(); //Pool sizes to create pool with

	//Create Descriptor pool
	checkValidResult(vkCreateDescriptorPool(mainDevice.logicalDevice, &poolCreateInfo, nullptr, &descriptorPool), "DESCRIPTOR POOL CREATION FAILED");
}
...
void VulkanRenderer::createDescriptorSets()
{
	...
	setAllocateInfo.descriptorSetCount = static_cast<uint32_t>(swapChainImages.size()); //Num sets to allocate
	setAllocateInfo.pSetLayouts = setLayouts.data(); //Layouts to allocate sets

	//Allocate descriptor sets
	checkValidResult(vkAllocateDescriptorSets(mainDevice.logicalDevice, &setAllocateInfo, descriptorSets.data()), "DESCRIPTION SETS ALLOCATION FAILED");

	//Update desriptor set buffer bindings
	for (size_t i = 0; i < swapChainImages.size(); ++i)
	{
		...
		vpSetWrite.dstSet = descriptorSets[i]; //Set to update
		vpSetWrite.dstBinding = 0; // Binding to update
		vpSetWrite.dstArrayElement = 0; //Index to update
		...
		//modelBufferInfo.buffer = modelDUniformBuffer[i]; //Buffer to get
		...
		//modelSetWrite.dstSet = descriptorSets[i]; //Set to update
		//modelSetWrite.dstBinding = 1; // Binding to update
		//modelSetWrite.dstArrayElement = 0; //Index to update
...
void VulkanRenderer::recordCommands(uint32_t currentImage)
{
	...
	checkValidResult(vkBeginCommandBuffer(commandBuffers[currentImage], &bufferBeginInfo), "RECORD TO COMMAND BUFFER FAILED");
	...
	for (size_t j = 0; j < meshList.size(); ++j)
	{
		...
		vkCmdBindVertexBuffers(commandBuffers[currentImage], 0, 1, vertexBuffers, offsets); //Commands to bind vertex buffer
	...
	checkValidResult(vkEndCommandBuffer(commandBuffers[currentImage]), "STOP RECORD TO COMMAND BUFFER FAILED");
}
...
void VulkanRenderer::allocateDynamicBufferTransferSpace()
{
	...
	////Create space in memory for dynamic buffer w/ alignment
...
{
	//Loop through options for compatible options
	...
	throw std::runtime_error("NO FORMATS FOUND");
}
	...
	VkImageUsageFlags useFlags, VkMemoryPropertyFlags propFlags, VkDeviceMemory* imageMemory)
{
	...
	imageCreateInfo.arrayLayers = 1; //num levels
	imageCreateInfo.format = format; //format tpe of image
	imageCreateInfo.tiling = tiling; //How image data should be tiled
	imageCreateInfo.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED; //layout of image data
	imageCreateInfo.usage = useFlags; //bit flags defining what image will be used for
	imageCreateInfo.samples = VK_SAMPLE_COUNT_1_BIT; //Num samples for multisampling
	imageCreateInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE; //Image can be shared between queues

	VkImage image;
	checkValidResult(vkCreateImage(mainDevice.logicalDevice, &imageCreateInfo, nullptr, &image), "IMAGE CREATION FAILED");
	...
	checkValidResult(vkAllocateMemory(mainDevice.logicalDevice, &memoryAllocateInfo, nullptr, imageMemory), "MEMORY ALLOCATION FOR IMAGE FAILED");
